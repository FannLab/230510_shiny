...
from script.PairNet import PairNetClassifier
#from model import Model
#from src.model import Model
from script.utils_temp import (
       load_dataset,
       permute,
       split,
       arr_to_dataloader,
...
#
################################################################################
def train(genotype_file,sample_file,train_id_file,OUT_DIR,SEED=None,EPOCHS = 20,MODEL_DIR = 'model/',log_file = 'log/log.txt'):
    np.random.seed(SEED)
    ...
    train_g, train_p, val_g, val_p, test_g, test_p = load_dataset(genotype_file, sample_file,
                                 train_sample_file=train_id_file )
    n_feature = train_g.shape[1]

    # geno, pheno = permute(geno, pheno)
    # train_g, val_g = split(geno, val_ratio = 0.2)
    ...
    train_loader = arr_to_dataloader(train_g, train_p, batch_size=256, shuffle=True)
    val_loader = arr_to_dataloader(val_g, val_p, batch_size=256, shuffle=False)
    test_loader = arr_to_dataloader(test_g, test_p, batch_size=256, shuffle=False)
